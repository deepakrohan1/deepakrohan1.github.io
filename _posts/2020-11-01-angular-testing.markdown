...
Once you have created a project you will end up with the below autogenerated test cases
...
describe('AppComponent', () => {
  ...
  it('should render title', () => {
    ...
    expect(compiled.querySelector('.content span').textContent).toContain('sample-testing-project app is running!');
  });
});
...
## Inside Configuration configure ATB
...
* ___imports___ Modules
* ___declarations___ Components
* ___providers___ Services
...
We have many scenarios where we kind have a `Promise` which needs to resolve when doing the test cases. Jasmine has it's own way handling these asynchronous execution - `spyOn`.

Angular provides `Async and fixture.whenStable` or `FakeAsync with tick()`
...
{% endhighlight %}
__1__ wrapping in async function block `async test zone`

__2__ when all the function stack is empty (all pending async methods are executed) it executes the promise
...
__1__ wrapping in a fakeAsync function

__2__ tick() to wait for all the async functions to complete

[Reference][reference]

[reference]: https://codecraft.tv/courses/angular/unit-testing/asynchronous/